...
interface SaldoContextData {
  ...
  const SaldoContext = createContext<SaldoContextData | undefined>(
    undefined
  );
  ...
  export const SaldoProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const atualizarSaldo = async () => {
      if (!userId) return;
      try {
        const novoSaldo = await saldoService.obterSaldo(userId);
    ...
    const deposito = async (valor: number) => {
      ...
      try {
        ...
        await saldoService.atualizarSaldo(userId, updated);
